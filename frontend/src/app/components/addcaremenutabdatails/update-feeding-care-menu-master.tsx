...
import { API_BASE_URL,API_BASE_URL_DEMO, GET_FEEDING_CARE_MENU_BY_ID, UPDATE_FEEDING_CARE_MENU, GET_FEEDING_MED_CARE_MENU, DRY_FOOD, WET_FOOD, WET_FOOD_QUANTITY, DRY_FOOD_QUANTITY
...
interface UpdateFeedingCareMenuProps {
    id?: number;  // id is expected, not initialId
    onSubmitComplete: (id: number | undefined) => void;  
    onBack: () => void;  
}

export default function UpdateFeedingCareMenu({ id: initialId, onSubmitComplete, onBack }: UpdateFeedingCareMenuProps) {
  const petId = localStorage.getItem('petid');
  const [id, setId] = useState<number | undefined>(initialId);  // initialId is the prop passed down from the parent component
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const response = await apiRequest(`${API_BASE_URL_DEMO}${GET_FEEDING_CARE_MENU_BY_ID}`, {
          method: 'POST',
        ...
        if (data) {
          setRun(data.run);
          setFoodInstructionsFirst(data.food_instructions_first);
          setFoodInstructionsSecond(data.food_instructions_second);
          setClientMedication(data.client_medication);
          setDailyHealthLogDate(data.daily_health_log_date);
          setDailyRun(data.daily_run);
          setPoo(data.poo);
          setPee(data.pee);
          setOther(data.other);
          setHealthCheck(data.health_check);
          setAction(data.action);
          setMidLitterCheck(data.mid_litter_check);
          setDay(data.day);
          setInitials(data.initials);
          setComments(data.comments);
          setMedData(data.medication_instructions);
          setFoodData(data.daily_food_intake);
        }
      } catch (error) {
        toast.error('Error fetching feeding care menu data');
      }
    };
  ...
  };// Depend on id and petId to refetch data when needed
...
const addFoodIntake = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiRequest(`${API_BASE_URL_DEMO}${UPDATE_FEEDING_CARE_MENU}`, {
        method: 'POST',
        body: JSON.stringify(requestData),
      });

      toast.success('Feeding data update successfully', {
        position: 'top-right',
        autoClose: 3000,
      ...
      toast.error('Error: Feeding data not update!', {
        position: 'top-right',
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      <div className="f2" id='datepickarea'>
        <div className="mb-3">
          ...
          {/* <input
            type="date"
            className="form-control"
            id="dailyHealthLogDate"
            value={dailyHealthLogDate}
            onChange={(e) => setDailyHealthLogDate(e.target.value)}
          /> */}
           <DatePicker
